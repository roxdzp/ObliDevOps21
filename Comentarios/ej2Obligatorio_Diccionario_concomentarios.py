...
#! ---------------------------------------------------------------------------- #
#!                   SECTION 1: Comunicación entre Python y Bash                   #
#! ---------------------------------------------------------------------------- #
...
#! Esta linea no la usamos en el script pero en lo que entregamos la pusimos, tenemos que saber que es.
    #? Decimos que se nos pasó borrarla de una vesión anterior del script?
#! os — Interfaces misceláneas del sistema operativo. Este módulo provee una manera versátil de usar funcionalidades dependientes del sistema operativo.
...
#! Se importa la función Popen del módulo subprocess de Python.
 #! NOPE! Subprocess es un módulo y Popen una clase (https://docs.python.org/3/library/subprocess.html#subprocess.Popen)
''' NOTE Python Class vs Module:
    The difference between a class and a module in python is that a class is used to define a blueprint for a given object,
    whereas a module is used to reuse a given piece of code inside another program. (https://www.pythonpool.com/python-class-vs-module/)
    '''
#! Subprocess: Permite generar nuevos procesos.
#! Popen: Permite ejecutar comandos de Linux en Python (permitirá ejecutar el script de la parte 1 del obligatorio).
#! PIPE: Para poder manejar la salida estándar y la salida estándar de errores del script del ejercicio 1.

import sys, argparse, re
#! sys: Útil para poder desplegar un mensaje en la salida estándar de errores.
''' NOTE[epic=sh&py] Ejemplo de sys
    INFORMACIÓN COMPLEMENTARIA, SOLO A MODO DE EJEMPLO, USÁNDOSE EL MODULO SYS PARA ACCEDER A LOS PARÁMETROS RECIBIDOS POR EL SCRIPT EN PYTHON.
    sys permite acceder a los parámetros recibidos por el script Python usando la variable sys.argv, que es una lista con los parámetros
    ...
    '''
#! argparse: Para interpretar los parámetros recibidos por este script Python, argument parser (agarra los argumentos y los digiere).
''' NOTE Acerca de argparse
     -  Es un equivalente al getops más "pro".
     -  Segun el profe, parseo es cuando agarramos determinado flujo de caracteres o secuencia de
         simbolos y hacer algun tipo de analisis/procesamiento/intepretacion de ese flujo.
     -  Facilita al usuario la utilización de argumentos en la linea de comandos. El programa define que
         argumentos son necesarios (obligatorios), cuales no son obligatorios, y cuales NO son argumentos válidos.
     -  Además el parse_args() nos permite generar automaticamente mensajes de ayuda sobre la utilización de los
         argumentos y controla los argumentos que sean invalidos.
    '''
#! re: Para utilizar expresiones regulares, debemos importar re.


## Guardamos en la variable "ej1_last" la ruta del script de bash que usaremos para obtener los valores a trabajar
ej1_last = ['/root/ObliDevOps21/ej1Obligatorio.sh']

## Utilizamos argparse (que importamos previamente), para poder trabajar los parámetros del script de Python.
#! Me guardo en la variable parser la funcion argparse.ArgumentParser() 
#! que es la funcion con la que voy a recibir los parámetros del python y los voy mandar al bash.
parser = argparse.ArgumentParser()
''' NOTE Explicación de parser = argparse.ArgumentParser()
...
## Creamos una función que nos convierte nuestra lista en una lista de diccionarios, donde las claves (keys) de ese 
...
## La variable claves (keys) se guarda en si misma el valor de todas las claves (keys) de uno de nuestros diccionarios de mi lista de
