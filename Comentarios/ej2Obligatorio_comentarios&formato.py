...
parser.add_argument("-r", "--recuento_horas",
                    help="Devuelve un contador de las horas/minutos/dias sumadas.", action="store_true")
...
parser.add_argument("-u", "--usuario", type=str, default=" ",
                    help="Nos permite buscar las horas de un usuario en particular, va a compañado de una variable que es el nombre de usuario.")
...
parser.add_argument("-o", "--orden", type=str, choices=["u", "t", "h", "d"],
                    help="Ordenar de forma creciente la salida del LAST por una de las cuatro columnas a seleccionar.")
...
parser.add_argument("-f", "--filtro", type=str, choices=[
                    "u", "t", "h", "f", "c", "n", "d"], help="Filtrado de listado (quita opciones).", nargs='+')

# El argumento -i nos permite definir si queremos invertir el orden de la lista recibida o no.
parser.add_argument(
    "-i", "--inverso", help="Devuelve el listado de los valores ordenados con el -o pero de forma inversa.", action="store_true")
...
def normalizar_lista():
    ...
    # Por cada valor en el rango (rango es la longitud total de la lista que retorna el script de bash).
    for i in range(1, rango):
        # Si la linea es vacia, no hago nada, en caso contrario la normalizo.
        if lista[i].split(" ")[0] != "":
            # Si la lista en esa posicion, separada por espacios y quedandome con la pocicion 0 es diferente de vacia, then...
            # A mi linea le reemplazo todos los espacios multiples y le pongo un solo espacio (esto es para poder splitear).
            linea = re.sub(' +', ' ', lista[i])
            # lines va a ser la sustitucion de espacios indefinidos por uno solo.
            # inicializo como vacia la fila que se va ir rellenando con los valores normalizados, osea "usuario  " "term         ", etc...
            fila = ""
            # Por cada longitud (nombre de mi variable, podia ser i, j, x, pepe o carlitos)
            for longitud in range(0, len(lista_longitudes)):
                ...
                # Si el valor  de nuestra columna (usuario, term, host, etc..) es menor
                if len(linea.split(" ")[longitud]) <= lista_longitudes[longitud]:
                    ...
                    # ljust agrega espacios a la derecha (left) de una cadena de
                    fila = fila + \
                        linea.split(" ")[longitud].ljust(
                            lista_longitudes[longitud])
                    # caracteres HASTA la cantidad asignada en la función.
                # Si nuestro valor de la columna (usuario, term, host, etc...) es mayor a 9, es porque debo recortar esa lina antes de cargarla en
                else:
                    # nuestra fila, para hacer eso utilizo [:hasta la longitud deseada]. Y este valor es cargado en nuestra fila.
                    # Si me pasa que la linea es más larga de lo que yo necesito, la corto, usando
                    fila = fila+linea.split(" ")[:lista_longitudes[longitud]]
                    # [:hastalalongituddeseada].
            # Al finalizar el for que recorre cada columna de neustra linea, genero una fila con los valores normalizados
...
def lista_diccionario(lista):
    ...
    # Por cada linea de mis lista de listas (osea por cada lista de mi lista de listas).
    for linea in lista:
        diccionario = {"Usuario": linea[:9], "Term": linea[9:22], "Host": linea[22:39], "Fecha": linea[39:43] +
                       linea[43:47]+linea[47:50], "H.Con": linea[50:56], "-": linea[56:58], "H.Des": linea[58:65], "T.Con": linea[65:72]}
        # Una vez que creo el diccionario, lo cargo en mi lista de diccionarios, de forma que al final obtengo
...
if args.orden == "u":
    # este Lambda recorre todos los diccionarios de mi lista de diccionarios y luego ordena en base a la clave que
    lista.sort(key=lambda ele: ele["Usuario"])
...
cabezales = ["Usuario  ", "Term         ", "Host             ",
             "Fecha      ", "H.Con ", "- ", "H.Des  ", "T.Con  "]
...
# Si no se ingresa el -f, entonces no hago nada. En caso contrario, empiezo a filtrar.
if args.filtro != None:
    ...
    for i in args.filtro:
        ...
        if i == "u":
            # Remuerve de mi lista de cabezales el cabezal Usuario, para que al imprimir los cabezales, no aparezca el filtrado.
            cabezales.remove("Usuario  ")
            for diccionario in lista:
                # Por cada diccionario en mi lista de diccionarios, elimino (popeo) la clave Usuario con su elemento correspondiente
...
# Por cada cabezal en mi lista de cabezales que ahora esta filtrada. Lo que debo hacer es crear una nueva linea (mis cabezales)
...
if normalizar_lista()[1] != "":
    ...
    if args.usuario:
        print("\nCantidad de conexiones listadas para el usuario",
              args.usuario+":", len(output[0].decode().split("\n"))-4)
    else:
        print("\nCantidad de conexiones listadas:",
              len(output[0].decode().split("\n"))-2)
    exit(0)
else:
    if args.usuario:
        print("\nCantidad de conexiones listadas para el usuario",
