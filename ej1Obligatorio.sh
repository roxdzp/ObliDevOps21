...
function gethoursminutes
{
    ...
    ## utilizando el grep grep "^$1 " (¿Por que un espacio después del $1, porque usamos tr -s " " delimitando cada columna por un espacio).
        hours=$(last | tr -s " " | grep "([0-9]*:[0-9]*)" | grep -Ev "^reboot\s+system\s+boot" | cut -d"(" -f2 | cut -d":" -f1)
        minutes=$(last | tr -s " " | grep "([0-9]*:[0-9]*)" | grep -Ev "^reboot\s+system\s+boot" | cut -d"(" -f2 | cut -d")" -f1 | cut -d":" -f2)
    else
        hours=$(last | tr -s " " | grep "([0-9]*:[0-9]*)" | grep "^$1 " | grep -Ev "^reboot\s+system\s+boot" | cut -d"(" -f2 | cut -d":" -f1)
        minutes=$(last | tr -s " " | grep "([0-9]*:[0-9]*)" | grep "^$1 " | grep -Ev "^reboot\s+system\s+boot" | cut -d"(" -f2 | cut -d")" -f1 | cut -d":" -f2)
    fi
    ...
    then
        ...
        if [ "$1" == "" ]
        then
            last | grep "([0-9]*:[0-9]*)" | grep -Ev "^reboot\s+system\s+boot"
        else
            last | grep "([0-9]*:[0-9]*)" | grep "^$1 " | grep -Ev "^reboot\s+system\s+boot"
        fi
        echo ""
    ...
    u )
        ...
        then
            ...
            then
                ...
                echo -e "Usuario  Term         HOST             Fecha      H.Con   H.Des  T.Con"
                last | grep "([0-9]*:[0-9]*)" | grep "^$OPTARG " | grep -Ev "^reboot\s+system\s+boot"
                exit 0
            fi
        fi
        exit 0
